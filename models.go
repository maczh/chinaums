...
// 表名映射（如需持久化到数据库）
func (BumBatchFileRecord) TableName() string {
	return "bum_batch_file_record" // 可根据实际需求修改表名
}

// BumBatchFileRespRecord 异步核销订单回盘文件明细行结构体（对应5.1.3回盘文件格式表格）
type BumBatchFileRespRecord struct {
	ID              uint   `gorm:"primaryKey;autoIncrement" json:"id"`                                // 自增主键
	CmdId           string `gorm:"column:cmd_id;type:varchar(30);not null" json:"cmdId"`              // 商户指令ID，M(必须出现且不为空)，长度30，与批量文件中核销指令ID一一对应
	SrcMerNo        string `gorm:"column:src_mer_no;type:varchar(14);not null" json:"srcMerNo"`       // 充值核销源企业用户号，M(必须出现且不为空)，定长14位，与指令文件内容一致
	MerOrderNo      string `gorm:"column:mer_order_no;type:varchar(30);not null" json:"merOrderNo"`   // 商户订单号，M(必须出现且不为空)，长度30，与指令文件内容一致
	VerifyAmt       string `gorm:"column:verify_amt;type:varchar(16)" json:"verifyAmt"`               // 核销金额，O(可选)，长度16位（单位分，数值类型），指商户核销指令中要求的金额
	TargetMerNo     string `gorm:"column:target_mer_no;type:varchar(16)" json:"targetMerNo"`          // 充值核销目标企业用户号，O(可选)，长度16位，与指令文件内容一致
	VerifyStatus    string `gorm:"column:verify_status;type:varchar(1);not null" json:"verifyStatus"` // 核销状态，M(必须出现且不为空)，长度1，0-失败，1-成功，2-已生成订单
	Remark          string `gorm:"column:remark;type:varchar(240)" json:"remark"`                     // 备注，C(特定环境下必须出现且不为空)，长度1-240，核销处理状态备注
	SubsidySerialNo string `gorm:"column:subsidy_serial_no;type:varchar(60)" json:"subsidySerialNo"`  // 补贴流水号，C(特定环境下必须出现且不为空)，长度1-60，如已生成订单则提供订单号
}
...
type BumBatchTransferFileRecord struct {
	...
	TargetMerNo       string `gorm:"column:target_mer_no;type:varchar(20);not null" json:"targetMerNo"`        // 充值核销目标企业用户号，M(必须出现且不为空)，定长14位（格式100010000******）
	...
	TransferredAmount int64  `gorm:"column:transferred_amount;type:bigint;default:0" json:"transferredAmount"` // 划付成功金额，单位为分
